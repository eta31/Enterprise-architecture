...
/*		Query query = entityManager.createQuery("select i from Item i, Category c where c.name = :categoryName and i item of c.items");*/
	...
	public List<Item> findItemCriteria(Integer initialPrice, User buyer, User seller) {
		...
		// initialize the "optional" predicates to false
		 Expression<Boolean> init = criteriaBuilder.literal(false);
		 Predicate buyerMatchPredicate= criteriaBuilder.isTrue(init);
		 Predicate sellerInitialPredicate = criteriaBuilder.isTrue(init); 
		    ...
		    if ((seller != null) && (initialPrice != null)  ) {
		        ...
		        Predicate and = criteriaBuilder.and(pricePredicate, sellerPredicate);
		        sellerInitialPredicate = criteriaBuilder.or(and);
	    ...
	    if ((buyer != null)) {
		    	...
		    	Predicate andBuyerPrice = criteriaBuilder.and(andBuyer,pricePredicate);
	
		    	buyerMatchPredicate = criteriaBuilder.or(andBuyerPrice);
 		    }
		    
	    
		 query.where(criteriaBuilder.or(sellerInitialPredicate,buyerMatchPredicate));
